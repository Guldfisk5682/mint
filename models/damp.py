"""DAMP (TL-UESTC/DAMP), adapted to shared-checkpoint SS-MTDA evaluation.

Upstream: https://github.com/TL-UESTC/DAMP, commit b4e82e2.
DomainNet memory adaptations follow the archived reproduction patch.
"""
...
class DAMP(TrainerXU):
    ...
    def check_cfg(self, cfg):
        ...
        if cfg.TEST.FINAL_MODEL != "last_step":
            raise ValueError("DAMP-MT must use the final checkpoint, not target-selected best_val")
    ...
    def build_data_loader(self):
        ...
        self.test_loaders_by_domain = {
            ...
            for domain, items in self.dm.dataset.test_by_domain.items()
        }
    ...
    def load_model(self, directory, epoch=None):
        ...
        model_file = "model.pth.tar-" + str(self.max_epoch)
    ...
    def test(self, split=None):
        """Evaluate one shared DAMP checkpoint on each target domain."""
        self.set_model_mode("eval")
        accuracies = []
        for domain, loader in self.test_loaders_by_domain.items():
            print(f"Evaluate on target domain: {domain}")
            self.evaluator.reset()
            for batch in loader:
                image, label = self.parse_batch_test(batch)
                output = self.model_inference(image)[0]
                self.evaluator.process(output, label)
            accuracy = self.evaluator.evaluate()["accuracy"]
            accuracies.append(float(accuracy))
            print(f"Target domain {domain} accuracy: {accuracy:.2f}%")
            self.write_scalar(f"test/{domain}_accuracy", accuracy, self.epoch)
        macro = sum(accuracies) / len(accuracies)
        print(f"Overall average: {macro:.2f}%")
        return macro
